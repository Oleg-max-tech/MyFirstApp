...
export const tmbdApi = {
  ...
  getMoviesByTypeAndGenre: async (type: string, genre: string | null) => {
    const url = genre
      ? `https://api.themoviedb.org/3/discover/${type}?with_genres=${genre}&api_key=YOUR_API_KEY`
      : `https://api.themoviedb.org/3/trending/${type}/week?api_key=YOUR_API_KEY`;
  ...
  searchTVShows: async (query: string) => {
    const response = await fetch(
      `https://api.themoviedb.org/3/search/tv?query=${query}&api_key=YOUR_API_KEY`
    );
    return response.json();
  },
};
